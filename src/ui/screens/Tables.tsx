...
import { isShadow } from '../../engine/world.ts';
import { PlayoffBracket, ShadowLeague } from './Playoffs.tsx';
...
export function Tables({ world, clubId, onPlayer, onClub }: { world: WorldState; clubId: number; onPlayer: (id: number) => void; onClub: (id: number) => void }) {
  ...
  const comp = world.competitions[compId]!;
  const shadow = isShadow(world, comp);
  ...
  // in Serie B con gli spareggi: 1ª-2ª su, 3ª-8ª playoff, 16ª-17ª playout, 18ª-20ª giù
  const po = comp.level === 2 && world.rules.playoffs;
  const upN = po ? 2 : comp.promote, downN = po ? 5 : comp.relegate; // in B con gli spareggi: la quota salvezza è sopra il playout
  const zone = (rank: number) => (rank <= (po ? 2 : comp.promote) ? 'up' : po && rank <= 8 ? 'po'
    : rank > n - (po ? 3 : comp.relegate) ? 'down' : po && rank > n - 5 ? 'pl' : '');
  const name = (rank: number) => world.clubs[full[rank - 1]?.clubId ?? -1]?.name ?? '';
  const safePts = downN > 0 ? full[n - downN - 1]?.pts ?? 0 : 0;
  ...
  return (
    <div className="stack">
      ...
      {shadow ? <ShadowLeague world={world} comp={comp} onClub={onClub} /> : <>
      <div className="kpis">
        {mine && (
          <div className="kpi">
        ...
        {comp.promote > 0 && (
          <div className="kpi"><span className="caps">{t(comp.level === 1 ? 'tables.title' : 'tables.promotion')}</span>
            <div className="big">1°{upN > 1 ? ` – ${upN}°` : ''}</div>
            <span className="muted">{Array.from({ length: upN }, (_, i) => name(i + 1)).join(', ')}</span>
        ...
        {downN > 0 && (
          <div className="kpi"><span className="caps">{t('tables.safety')}</span>
            <div className="big">{n - downN}°<small>{t('tables.safePts', { pts: safePts })}</small></div>
            <span className="muted">{name(n - downN)}</span></div>
        )}
        {downN > 0 && (
          <div className="kpi"><span className="caps">{t(po ? 'tables.relegationPo' : 'tables.relegation')}</span>
            <div className="big">{n - downN + 1}° – {n}°</div>
            <span className="muted">{Array.from({ length: downN }, (_, i) => name(n - downN + 1 + i)).join(', ')}</span>
      ...
      <div className="cols2">
        <div className="panel">
          ...
          <div className="row wrap muted small">
            ...
            {po && <span className="zone-po-key">▌ {t('playoff.zone')}</span>}
            {po && <span className="zone-pl-key">▌ {t('playout.zone')}</span>}
            {comp.relegate > 0 && <span className="pos-bad">▌ {t('tables.relegation')}</span>}
            <span>{t('tables.tiebreak')}</span>
          </div>
        </div>
        <div className="stack">
          {comp.level === 2 && <PlayoffBracket world={world} me={clubId} />}
