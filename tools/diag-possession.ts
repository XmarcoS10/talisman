// Possesso di una forte contro una debole (Blocco 2b): a tempo, come lo misura il motore, e a quota di passaggi, come Opta.
...
let time = 0, share = 0, shots = [0, 0], passes = [0, 0]; const N = 400;
for (let i = 0; i < N; i++) {
  ...
  time += s.possession; share += s.passes / (s.passes + t.passes) * 100; shots[0] += s.shots; shots[1] += t.shots; passes[0] += s.passes; passes[1] += t.passes;
}
console.log(`2ª contro 19ª: possesso a tempo ${(time / N).toFixed(1)}% · a quota di passaggi (Opta) ${(share / N).toFixed(1)}% · tiri ${(shots[0] / N).toFixed(1)} contro ${(shots[1] / N).toFixed(1)} · passaggi ${(passes[0] / N).toFixed(0)} contro ${(passes[1] / N).toFixed(0)}`);
